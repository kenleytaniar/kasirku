...
<?php 
  ...
  <tr>
    ...
    include 'email.php';
      ...
      <?php if ($d['Stok_barang'] == 2 || $d['Stok_barang'] == 1 ){?>
        <div class="alert alert-warning alert-sm" role="alert">
          Stock hampir  habis 
        </div>
        <?php } ?>

        <?php if ($d['Stok_barang'] > 2){?>
...
if(isset($_POST['TambahProduk']))
{
    ...
    $Stok_barang = htmlspecialchars($_POST['Tambah_Stok_barang']);
    ...
    $InputProduk = mysqli_query($conn,"INSERT INTO produk (kode_produk,nama_produk,Stok_barang,harga_jual)
     values ('$kodeproduk','$namaproduk','$Stok_barang','$harga_jual')");
    if ($InputProduk){
...
if(isset($_POST['KurangStock'])){
$kode_produk = $_POST['kode_produk'];
$jumlah_beli = $_POST['jumlah']; // Jumlah barang yang ingin dibeli

// Mengeksekusi query untuk mengambil stok dari database
$sql = "SELECT Stok_barang FROM produk WHERE kode_produk='$kodeproduk'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $stok_sekarang = $row["Stok_barang"];

    // Pastikan stok mencukupi
    if ($stok_sekarang >= $jumlah_beli) {
        $stok_terbaru = $stok_sekarang - $jumlah_beli;

        // Mengeksekusi query untuk memperbarui stok ke dalam database
        $sql_update = "UPDATE produk SET Stok_barang = $stok_terbaru WHERE kode_produk='$kodeproduk'";
...
if(isset($_POST['SimpanEdit'])){
    ...
    $Stok_barang1 = htmlspecialchars($_POST['Edit_Stok_barang']);
    ...
    if($HasilData > 0){
        ...
        nama_produk='$namaproduk1',Stok_barang='$Stok_barang1',harga_jual='$harga_jual1'
  ...
  $stok = mysqli_fetch_assoc(mysqli_query($conn,"SELECT qty FROM laporan WHERE kode_produk=$kodeproduk"));
