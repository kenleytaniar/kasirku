...
<div class="bg-light p-3" style="border-radius:0.25rem;">
  <div class="row gy-3 align-items-center row-home">
    ...
    <div class="col-md-4 col-lg-6 mb-2 text-primary text-right">
      ...
      <h1 class="mb-0" style="font-weight:600;">Rp. <?php echo ribuan($itungtrans3) ?></h1>
     <?php  
     ?>
    </div>
...
while ($rowCart = mysqli_fetch_assoc($queryCart)) {
    ...
    if ($jumlah >= 1) {
        ...
        if ($query_update) {
            echo "Stok barang berhasil diperbarui di database<br>";
        } else {
...
mysqli_free_result($queryCart); // Bebaskan hasil kueri
...
elseif (isset($_GET['hapusAll'])) {
  // Tombol "Hapus Semua" ditekan

  // Implementasikan logika untuk mengembalikan stok

  // Ambil data keranjang
  $queryCart = mysqli_query($conn, "SELECT kode_produk, qty FROM cart");
  while ($rowCart = mysqli_fetch_assoc($queryCart)) {
    $barang = htmlspecialchars($_POST['Ckdproduk']);
    $jumlah = htmlspecialchars($_POST['jumlah']);

    // Panggil fungsi untuk mengembalikan stok
    kembalikanStok($barang, $jumlah);
  }
...
<!-- end Modal Exit -->
    ...
    <script type="text/javascript">
      <?php echo $jsArray,$jsArray1,$jsArray2; ?>
        ...
        var jumlah_beli =  parseInt(document.getElementById('jumjum').value);
